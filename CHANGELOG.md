...
- page creation as "markdown" generated a not well formatted content
...
- `upgrade` command added to easily upgrade existing sveltin project to the latest sveltin release. From now on, each new sveltin release will make use of it. With release `0.11.0` we changed the content for some autogenerated files (e.g. `defaults.ts`, `theme.config.js` ...) to make use of the new `sveltin.json` file. We replaced some npm packages too (e.g. `remark-external-links` with `rehype-external-links`). By running `sveltin upgrade project` your project will make use of the latest introduced by version `0.11.0`. It works with migrations. As per today it applies migrations to the following files
  ...
  - `mdsvex.config.js`: replace config for `remark-external-links` with the ones form `rehype-external-links`
  - `svelte.config.js`:
    ...
    - remove `config.kit.prerender.enabled` as per [#7762](https://github.com/sveltejs/kit/pull/7762) merged into [1.0.0-next.554](https://github.com/sveltejs/kit/blob/master/packages/kit/CHANGELOG.md#100-next554)
...
- SvelteKit supported version bumped to `1.0.0-next.559`
- deps updated for tailwindcss projects
