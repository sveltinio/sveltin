...
A new `upgrade` command has been added. Its goal is to easily upgrade existing sveltin project to the latest sveltin release. From now on, each new sveltin release will make use of it.

With release `0.11.0` we changed the content for some autogenerated files (e.g. `defaults.ts`, `theme.config.js` ...) to make use of the new `sveltin.json` file. We replaced some npm packages too (e.g. `remark-external-links` with `rehype-external-links`).

By running `sveltin upgrade project` your project will make use of the latest introduced by version `0.11.0` and compliant with the supported sveltekit version.

`upgrade` works with migrations. As per today it applies migrations to the following:
...
| `package.json`              | replace `remark-external-links` with `rehype-external-links` and bump sveltekit version to v1.0.0 |
