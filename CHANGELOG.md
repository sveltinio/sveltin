...
- logo on `Footer.svelte` when theme is _sveltin_ not properly loaded
...
`@sveltejs/kit` and `@sveltejs/adapter-static` upgrated to `v1.0.5`
...
- the content for some autogenerated files (e.g. `defaults.ts`, `theme.config.js` etc.) has been updated to make use of the new `sveltin.json` file.
...
| `package.json`                      | replace `remark-external-links` with `rehype-external-links` <br/> bump sveltekit to v1.2.6 and some other deps |
| `mdsvex.config.js`                  | replace `remark-external-links` with `rehype-external-links` <br/> replace `headings.js` with `@sveltinio/remark-headings` <br/> remove `remark-slug` import and usage |
...
| `src/routes/**/+layout.svelte`      | replace props and types on SEO components from `@sveltinio/seo`                                                 |
| `src/routes/**/+page.[svelte\|svx]` | replace props and types on SEO components from `@sveltinio/seo` <br/> replace `data-sveltekit-prefetch` with `data-sveltekit-preload-data="hover"` as per sveltekit |
