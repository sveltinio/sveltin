...
## [v0.11.0](https://github.com/sveltinio/sveltin/releases/tag/v0.11.0) (2022-12-19)

[Full Changelog](https://github.com/sveltinio/sveltin/compare/v0.10.1...v0.11.0)

This time we are not going to use the autogenerated changelog! There are remarkable changes that require a few more words.
...
- logo on Footer.svelte when theme is "sveltin" not properly loaded
- import string for ScrollToTopButton component on `+layout.svelte` when `sveltin` theme.

### Breaking

- Some types from `sveltin.d.ts` file are changed. Most of them were interfaces before, now are types. We did not provided any migration (see **[Migrate](#migrate)** section below) to it. You could have adapted the file content to reflect some introductions on your side and would be tough auto migrate. New sveltin projects will use the new types. For the existing ones please, refer to [sveltin.d.ts](https://github.com/sveltinio/sveltekit-static-starter/blob/main/src/sveltin.d.ts) file and adapt it to your existing file.
...
`@sveltejs/kit` and `@sveltejs/adapter-static` upgrated to `v1.0.0`

Congrats and a big thank to the Svelte team 👏.

#### sveltin.json

When scaffolding a new project, a new`sveltin.json` file is added. It simplify settings across parts of the project. The overall idea is to use it as unique config file. E.g. When nodejs importing JSON module will not be an experimental feature anymore, we could think to use `sveltin.json` as unique config file for settings to be passed to `svelte.config.js` or `vite.config.[js|ts]`

#### Migrate

A new `migrate` command has been added. Its goal is to easily upgrade/migrate existing sveltin project to the latest sveltin release. From now on, each new sveltin release will make use of it.

> `sveltin migrate` command has nothing to do with the `svelte-migrate` [package](https://www.npmjs.com/package/svelte-migrate). `sveltin migrate` command takes care os sveltin related files. You do not need to use `svelte-migrate` because your routes etc. generated by sveltin commands are alreasy compliants with the latest changes introduced to SvelteKit.

With release `0.11.0` we changed the content for some autogenerated files (e.g. `defaults.ts`, `theme.config.js` etc.) to make use of the new `sveltin.json` file. We replaced some npm packages too (e.g. `remark-external-links` with `rehype-external-links`).
...
- `@sveltejs/kit` upgraded to `v1.0.0`
- `@sveltejs/adapter-static` upgraded to `v1.0.0`
- `vite` upgraded to `v4.0.1`
- js deps updated (`typescript`, `eslint`,`vite-plugin-svelte` etc.)
- `data-sveltekit-prefetch` replaced by `data-sveltekit-preload-data="hover"` for metadata pages and slug pages
- `data-sveltekit-prefetch` replaced by `data-sveltekit-preload-data="hover"` in `app.html`
